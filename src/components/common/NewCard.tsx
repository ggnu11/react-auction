...
const NewCard = ({ title, data, colSpan, cardStyle, imgStyle }: NewCardProps) => {
  return (
      ...
      <Row gutter={[16, 16]}>
        {data?.map((item) => (
          <Col key={item.id} {...colSpan}>
            <Card hoverable className={cardStyle} cover={<img alt={item.name} src={item.image} className={imgStyle} />}>
